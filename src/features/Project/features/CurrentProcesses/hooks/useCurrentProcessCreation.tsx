...
const useCurrentProcessCreation = (cloneId?: string): UseCurrentProcessCreationReturn => {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const { project } = useProject()
  const { response, doFetch, retry, loading, error } = useFetch(
    `${config.api.msProcesses.baseUrl}/projects/${project?.id}/current-processes`,
    {
      method: 'POST',
    }
  )
  ...
  const onSubmit = useCallback(
    (data: any) => {
      void doFetch({ data: { ...data, cloneId } }).then((response) => {
...
export default useCurrentProcessCreation
