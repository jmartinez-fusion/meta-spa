...
const useCurrentProcessEdit = (currentProcessId: string): UseCurrentProcessCreationReturn => {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const { project } = useProject()
  const { response, doFetch, retry, loading, error } = useFetch(
    `${config.api.msProcesses.baseUrl}/projects/${project?.id}/current-processes/${currentProcessId}`,
    {
      method: 'PUT',
    }
  )
...
export default useCurrentProcessEdit
