...
export interface UseContinueSurveyResult {
  onClickOpenConfirm: (survey: Survey) => void
}

export default function useContinueSurvey(refresh: () => void): UseContinueSurveyResult {
    ...
    (survey: Survey) => {
      ...
      confirm({
        ...
        .then(() => {
          void doFetch({
            method: 'POST',
            url: `${config.api.msSurveys.baseUrl}/projects/${project?.id}/surveys/${survey.id}/continue`,
          }).then(() => {
            refresh()
          })
        })
